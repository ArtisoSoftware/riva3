  ...
  <div>
    ...
    <div v-show="!showData" class="text-center text-h5">
      <div class="text-black q-pt-md">
        <b
          >How is {{ dataInput.exportingName }}'s contribution to exports
          production in {{ dataInput.importingName }} distributed across
          sectors?</b
        >
      </div>
...
const totalSmall = ref(0);
const totalSmallText = ref("");
...
const loadData = async () => {
  ...
  let allsector = dataResult[0];
  totalSmall.value = Number(allsector.value);
  if (Number(totalSmall.value) > 1000) {
    totalSmallText.value =
      (Number(totalSmall.value) / 1000).toFixed(2) + " billion";
  } else {
    totalSmallText.value = Number(totalSmall.value).toFixed(2) + " million";
  }
  ...
  let topResult1 =
    ...
    " (" +
    ((Number(dataResult[0].value) / totalSmall.value) * 100).toFixed(2) +
    "%)";
  let topResult2 =
    dataResult[1].exp_sector +
    " (" +
    ((Number(dataResult[1].value) / totalSmall.value) * 100).toFixed(2) +
    "%)";
  let topResult3 =
    dataResult[2].exp_sector +
    " (" +
    ((Number(dataResult[2].value) / totalSmall.value) * 100).toFixed(2) +
    "%)";
  let topResult4 =
    dataResult[3].exp_sector +
    " (" +
    ((Number(dataResult[3].value) / totalSmall.value) * 100).toFixed(2) +
    "%)";
  let topResult5 =
    dataResult[4].exp_sector +
    " (" +
    ((Number(dataResult[4].value) / totalSmall.value) * 100).toFixed(2) +
    "%)";
  ...
  setTimeout(() => {
    chartRef.value = Highcharts.chart("container3", {
      ...
      title: {
        ...
        text: `How is ${dataInput.value.exportingName}'s contribution to export production in ${dataInput.value.importingName} distributed across sectors?`,
      },
      subtitle: {
        useHTML: true,
        style: {
        ...
        text: `Gross exports of ${dataInput.value.exportingName} to ${dataInput.value.importingName} amount to $${totalLarge.value} in ${dataInput.value.year}. Of these exports, $${totalSmallText.value} is used by  ${dataInput.value.importingName} for its own export production, and come mainly from the following exporting sectors in  ${dataInput.value.exportingName}: ${topResult1}, ${topResult2}, ${topResult3}, ${topResult4}, ${topResult5}<br/><br/>
          ${dataInput.value.exportingName}'s constribution to ${dataInput.value.importingName}'s export production:  $${totalSmallText.value} / ${dataInput.value.exportingName}'s gross exports to ${dataInput.value.importingName} :   $${totalLarge.value}
          `,
        align: "left",
      },
      exporting: {
